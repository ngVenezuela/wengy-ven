...
type ChatTypes = 'admin' | 'main' | 'private';

export const getChatType = (message: Message): ChatTypes => {
  const {
    chat: { id: chatId },
  } = message;
 ...
 */
export const sendGroupId = async ({
  id: chatId,
  type,
}: Chat): Promise<void> => {
...
export const verifyUrls = async (message: Message): Promise<void> => {
